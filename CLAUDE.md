...
12. **Acceptance test must include 10-match reality check.** No phase that touches the prediction page or the model output is closed without manually walking through 10 current upcoming matches in the dashboard and recording: (model prob, market prob, surface-Elo prob, "why model differs" panel triggered correctly, subjective 1-5 rating of analyst usefulness). Average rating < 3.5 OR any inverted favourite that the dashboard doesn't explain = phase reopened. Phase 6.2 added this rule because Phase 4's average-Brier acceptance test masked tail-failure modes that only show up on a per-match walkthrough.
   ...
   **`why_model_differs` rule set (Phase 6.2 follow-up):** the panel ran on three rules originally (activity gap / stale Elo / career asymmetry) and stayed silent on cases where none fired (e.g., Baez-Burruchaga: both active, both with fresh Elo, both mid-tour — yet model + surface-Elo both pick the lower-ranked underdog against a rank-driven market). The rule set is now five + an "unexplained" generic fallback: (1) activity gap, (2) stale surface Elo, (3) **surface-Elo agrees with model against market** by ≥ 5pp on both signals (the rank-vs-specialist case), (4) **recent-form gap** on the surface ≥ 30pp over the last 60 days, (5) career asymmetry, (6) explicit "unexplained" message when none fire but the page-level 10pp trigger already asked for an explanation — points the user at H2H / recent form / news blocks below. Rule #12 (no unexplained inverted favourite) is structurally enforceable: the panel can never go silent on a > 10pp gap.
...
- **Tavily request shape (Phase 6.2 follow-up):** `topic="news"` + `days=32`. Switches Tavily from the general-web index to the news-only index AND applies a server-side recency cutoff. Phase 6.1 used `search_depth="basic"` only and relied entirely on the post-validate filter; live use surfaced stale items (e.g., a January Australian Open injury showing up for a May Roland Garros match) because Tavily's `published_date` is often null and our filter kept null-date items. Pushing the recency cutoff into Tavily itself eliminates most stale leaks at the source. Cost unchanged (same per-query billing).
...
- **Preferred sources** (mentioned in system prompt, not whitelist-enforced): **ESPN, BBC, tennis.com, tennis365.com**, X/Twitter when surfaced, plus journalist accounts and reddit. Tour-official `atptour.com` / `wtatennis.com` are intentionally not highlighted — they surface only news everyone already has.
...
- **Recency window:** the last **32 days** (Phase 6.1, was 14 in Phase 5). Enforced three ways: (a) Tavily `days=32` server-side filter (Phase 6.2 follow-up); (b) system-prompt instruction with explicit generic drop patterns for stale-tournament references; (c) post-validate filter in `agent.py` drops `NewsItem`s with parsed `published_date > 32 days before match_date`. Items with `published_date = None` are kept ONLY when the snippet/title carries explicit recency anchors (current tournament + recent score / round / "today" / "this week"); the system prompt requires the agent to drop date-unknown items lacking such anchors.
- **Prompt-side stale-item rules (Phase 6.2 follow-up):** the system prompt lists four generic drop patterns rather than name-specific examples (the earlier name-specific examples overfit to Monfils/Gaston and didn't transfer): (1) tournament-edition references older than the active edition — using the known calendar slot (AO ≈ January, RG ≈ late-May, Wimbledon ≈ July, US Open ≈ Aug-Sep); (2) "retires/withdraws at <tournament>" without an edition disambiguator AND null `published_date`; (3) multi-year career retrospectives; (4) name-collision items (Arthur Fils ≠ Gael Monfils, Alex de Minaur ≠ Alex Michelsen) — verify character-for-character on first+last name before including.
...
## Hot-data refresh (matchstat) — per-tournament refactor (Phase 6.2 follow-up)

- **Endpoint choice:** `refresh_hot` no longer iterates `/fixtures/{date}` per day. It first discovers active tour-level tournament IDs via a single `/fixtures/{today}` probe (filtered to `tournament.rank.name in {"Grand Slam", "Main tour"}`), then pulls each tournament's full draw via `/fixtures/tournament/{id}`. Slam R1 fixtures whose Order of Play hasn't been announced for their specific day still surface (Roland Garros publishes day-N's schedule the evening before; per-date refresh systematically missed those rows).
- **Cost on a typical week:** ~7-9 matchstat credits per refresh (1 calendar + 1-2 discovery + 3-5 per-tournament + rankings × 2 + Slam-results × 1-2), down from ~13 in the per-date loop.
- **`fixture_lookahead_days` is a prune-window parameter only.** Default 3. It no longer drives the fixture-fetch loop (per-tournament returns everything), but the prune passes use `today + lookahead + 2` as the upper bound when sweeping stale rows.
- **`tournament_tier` fallback:** matchstat's `calendar` is forward-only and silently drops currently-active tournaments. To get a tier for Roland Garros mid-tournament we fall back to the fixture's `tournament.rank.name` ("Grand Slam" / "Main tour") when the calendar lookup misses. This is what lets `_prune_completed_slam_fixtures` find active Slam tournament IDs.
- **Four prune passes** after every refresh (run in this order, all DELETE rows):
  1. `_prune_stale_scheduled_matches` — fixture IDs not seen in this run AND in the active date window. Removes rows matchstat stopped returning.
  2. `_prune_contradicted_round_fixtures` — if player X is in both R1 and R2 at the same tournament, R1 is stale (X already won it).
  3. `_prune_duplicate_matchups` — same (tour, tournament, sorted-player-pair, round) under multiple fixture IDs → keep the most-recently-ingested. matchstat **re-uses fixture IDs** for different matchups over time (observed: Popyrin-Svajda took over fx_id=1271 from Griekspoor-Arnaldi), so the `ON CONFLICT DO UPDATE` clause also refreshes player names + external IDs.
  4. `_prune_completed_slam_fixtures` — for active Grand Slam tournaments only, fetch `/tournament/results/{id}` and DELETE any scheduled-matches row whose matchup appears as completed. Catches the leaked-completed-fixture case where no later-round contradiction exists yet (e.g., Duckworth-Diallo R1 published as upcoming on Tuesday after being played on Sunday).
- **ON CONFLICT DO UPDATE refreshes player names + external IDs** (Phase 6.2 follow-up). matchstat reuses `fixture_external_id` for different matchups; if we don't refresh names on conflict the stale matchup lingers and the new one is silently lost.
- **Manual refresh button on Home:** Streamlit "Refresh fixtures" button triggers `refresh_hot(...)` synchronously; costs ~7-9 credits per click and supersedes the cron-driven daily refresh when the user wants fresh schedule mid-day.
...
- **Display:**
  - **Home (upcoming matches) — date only, no time** (Phase 6.2 follow-up). matchstat returns `T12:00:00.000Z` for most Slam outside-court fixtures as a day-level placeholder, indistinguishable from a genuine 11:00 CEST start. Rendering all of them as "11:00 CEST" misled — 17 matches at the same hour is physically impossible across only a handful of courts. We now render only `"Tue, May 26"` everywhere in the Home upcoming list. The 5h time-based lookback that combined with the placeholder to drop matches mid-day was replaced with a `DATE(scheduled_start_utc) >= today_utc` filter — matches stay visible all day regardless of when matchstat claims they start.
  - **Match dashboard (per-fixture) and other surfaces** can still call `format_match_time_for_display(utc_dt)` (returns `"Sun, May 25 — 11:00 CEST (09:00 UTC)"`) when a time is genuinely useful and the user is on the page actively watching one match. CEST is shown as the user-facing TZ; UTC kept in parentheses as the unambiguous reference. Never label naive timestamps as "UTC" in the UI.
